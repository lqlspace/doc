...
# 数据类型
* 可以使用DECIMAL(M,D)类型存储金额，进行高精度运算，FLOAT和DOUBLE类型会出现误差；
* char(M)最长只有255，varchar和text最长为65535个字节，尽量使用varchar，因为使用text会使DML操作变得很慢；
* 日期时间，用DATETIME（8字节）、TIMESTAMP（4字节）均可；
## 有一些公司用分而不是用元来表示金额，存储类型是int。这种方式和decimal哪个比较好呢？
