## 背景  
> 执行brew命令安装应用的时候，跟以下3个仓库地址有关，而由于不可抗力因素，
>"brew install 应用"会异常缓慢甚至超时，为了提供办公效率，须切换成国内镜像：  
```cassandraql
brew.git
homebrew-core.git
homebrew-bottles
...
```
由于被墙，此命令经常出现"拒绝连接"，贴出Install脚本如下：
```cassandraql
#!/usr/bin/ruby
...
Core tap HEAD: 8d295b1def42ab3d5e3592ddc9912f2fa2b569de
...
### 更换homebrew-bottles
- 查看使用的shell，根据版本不同，会输出2种结果，/bin/zsh 或 /bin/bash，根据类型进行操作即可:
```cassandraql
// zsh
